<?php
	...
	$query = "SELECT * FROM anggota INNER JOIN daftar_pkt ON anggota.nim = daftar_pkt.nim ";
	...
	$pdf->Line(10,25,182,27);
	// $pdf->Line(10,30,200,30);
	$pdf -> SetFont("Times", "", 12);
	$pdf->Cell(0,8,'Yang bertanda tangan di bawah ini menyatakan bahwa mahasiswa Departemen Kimia Fakultas Sains',0,1,'L');
	$pdf->Cell(50,5,'dan Matematika Universitas Diponegoro Semarang',0,'L');
	$pdf->Line(50,25,182,97);
	$pdf->Cell(50,5,'Waktu',0,'L');$pdf->Cell(5,5,':',0,'C');$pdf->Cell(135,5,'waktu ujian berlangsung WIB',0,1,'L');
	$pdf->Cell(50,5,'Tempat',0,'L');$pdf->Cell(5,5,':',0,'C');$pdf->Cell(135,5,'<tempat_ujian>',0,1,'L');
	$pdf->Cell(50,5,'Nama/ NIM',0,'L');$pdf->Cell(5,5,':',0,'C');$pdf->Cell(135,5,'<nama mahasiswa> / <nim>',0,1,'L');
	$pdf->Cell(50,5,'Pembimbing',0,'L');$pdf->Cell(5,5,':',0,'C');$pdf->Cell(5,5,'1.',0,'C');$pdf->Cell(130,5,'',0,1,'L');
	$pdf->Cell(50,5,'',0,'L');$pdf->Cell(5,5,':',0,'C');$pdf->Cell(5,5,'2.',0,'C');$pdf->Cell(130,5,'',0,1,'L');
	$pdf->Cell(50,5,'Judul Skripsi',0,'L');$pdf->Cell(5,5,'',0,'C');$pdf->MultiCell(135,5,'Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industrys standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book.',0,'L');

	// $pdf->Cell(0,5,'',0,1,'L');
	// $pdf->Cell(9,12,'NO',1,0,'C');$pdf->Cell(45,12,'NAMA',1,0,'C');$pdf->Cell(40,12,'NIM',1,0,'C');$pdf->Cell(75,12,'PILIHAN LABORATORIUM',1,0,'C');$pdf->Cell(20,12,'TTD',1,1,'C');
	// // $query = "SELECT * FROM anggota INNER JOIN daftar_pkt ON anggota.nim = daftar_pkt.nim ";
	// 	$pdf->Cell(0,5,'PENDAFTARAN PRAKTIKUM KIMIA TERPADU',0,1,'L');
	// $pdf->Cell(0,5,'SEMESTER GENAP TAHUN AKADEMIK 2016 / 2017',0,1,'L');		
	// $pdf->Cell(0,5,'DEPARTEMEN KIMIA FAKULTAS SAINS DAN MATEMATIKA',0,1,'L');
	// $pdf->Cell(0,5,'UNIVERSITAS DIPONEGORO SEMARANG',0,1,'L');

	$pdf -> SetFont("Times", "B", 12);
	$pdf->Cell(50,5,'Penilaian',0,'L');$pdf->Cell(5,5,'',0,'C');$pdf->Cell(135,5,'Antara 1 sampai dengan 100',0,1,'L');
	...
	$pdf->Cell(5,6,'I.','L,B',0,'C');$pdf->Cell(0,6,'Skripsi (40%)','R',1,'L');
	$pdf -> SetFont("Times", "", 12);
	...
	$pdf->Cell(5,6,'II.','L,B',0,'C');$pdf->Cell(0,6,'Presentasi(40%)','R',1,'L');
	$pdf -> SetFont("Times", "", 12);
	...
	$pdf->Cell(5,6,'III.','L,B',0,'C');$pdf->Cell(0,6,'Diskusi (40%)','R',1,'L');
	$pdf -> SetFont("Times", "", 12);
