					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered table-hover" id="tabelku">
						  ...
						  <?php
						    ...
						    if (($status=="petugas")) {
							      $query = "SELECT * FROM tr1 p INNER JOIN mahasiswa m ON p.nim=m.nim LEFT JOIN dosen d ON p.nip1=d.nip	WHERE p.nilai_tr1 IS NOT NULL ORDER BY m.nim LIMIT 10";
							    }elseif ($status=="anggota"){
							      $query = " SELECT * FROM tr1 p INNER JOIN mahasiswa m ON p.nim=m.nim LEFT JOIN dosen d ON p.nip1=d.nip WHERE m.nim='".$anggota->nim."'";
							    }elseif ($status=="dosen"){
							      $query = "SELECT * FROM tr1 p INNER JOIN mahasiswa m ON p.nim=m.nim LEFT JOIN dosen d ON p.nip1=d.nip WHERE p.nip1='".$dosen->nip."' ORDER BY nama LIMIT 10";
							    }elseif($status=="lab") {
							      $query = "SELECT * FROM tr1 p INNER JOIN mahasiswa m ON p.nim=m.nim LEFT JOIN dosen d ON p.nip1=d.nip INNER JOIN lab ON p.idlab_tr1=lab.idlab WHERE p.idlab_tr1='".$lab->idlab."' ORDER BY nama LIMIT 10";
						    }
