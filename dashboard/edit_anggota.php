	...
	if(!isset($_POST['edit'])){
      ...
      // echo "<br>";
		if ($email=='') {
			...
			$query0 = "SELECT email FROM mahasiswa WHERE nim='".$nim."'";
    	$result0 = $con->query($query0);
    	$rowz = $result0->fetch_object();
    	$email_lama = $rowz->email;
			if ( (($count_mhs == 0)|| ($email_lama==$email)) && ($count_dosen == 0) && ($count_admin == 0) && ($count_lab == 0) ) {
        $validEmail = TRUE;
    	} else {
...
<div class="row">
	<div class="col-md-6">
					...
					<div class="col-md-12">
						<form method="POST" role="form" autocomplete="on" action="">
							...
							<div class="form-group">
								...
								<input class="form-control" type="email" name="email" size="30" placeholder="example@email.com" required value="<?php if(isset($email_lama))echo $email_lama; else echo($email) ?>">
							</div>
							<div class="form-group">
		...
		<a href="daftar_anggota.php"><button class="btn btn-info">Kembali ke Daftar Anggota</button>
</div>
