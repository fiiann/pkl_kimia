					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered table-hover" id="tabelku">
						  ...
						  <?php
						    // Assign a query
						    if ($status=="petugas") {
						      $query = "SELECT * FROM pkt p INNER JOIN mahasiswa m ON p.nim=m.nim INNER JOIN dosen d ON p.dosen_pembimbing=d.nip INNER JOIN lab ON p.flag_lab=lab.idlab WHERE p.nilai<>0 ORDER BY m.nama LIMIT 10";
						    }elseif ($status=="dosen"){
						      $query = "SELECT * FROM pkt p INNER JOIN mahasiswa m ON p.nim=m.nim INNER JOIN dosen d ON p.dosen_pembimbing=d.nip INNER JOIN lab ON p.flag_lab=lab.idlab WHERE p.nilai<>0 AND p.dosen_pembimbing=$dosen->nip ORDER BY m.nama LIMIT 10";
