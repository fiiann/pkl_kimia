					...
					<div class="table-responsive">
						<table class="table table-striped table-bordered table-hover" id="tabelku">
							...
							<?php
								...
								if (($status=="petugas")) {
									$query = "SELECT * FROM pkt p INNER JOIN mahasiswa m ON p.nim=m.nim LEFT JOIN dosen d ON p.dosen_pembimbing=d.nip	WHERE p.dosen_pembimbing IS NULL ORDER BY nama LIMIT 10";
								}elseif ($status=="anggota"){
									...
									$query = "SELECT * FROM pkt p INNER JOIN mahasiswa m ON p.nim=m.nim LEFT JOIN dosen d ON p.dosen_pembimbing=d.nip WHERE d.nip='".$dosen->nip."' ORDER BY nama LIMIT 10";
								}elseif($status=='lab') {
									$query = "SELECT * FROM pkt p INNER JOIN mahasiswa m ON p.nim=m.nim LEFT JOIN dosen d ON p.dosen_pembimbing=d.nip	WHERE p.dosen_pembimbing IS NULL AND p.flag_lab=$lab->idlab ORDER BY nama LIMIT 10";
								}
								...
								while($row = $result->fetch_object()){
									...
									if ($status=="anggota") {
										...
										if($row->dosen_pembimbing == null){
											echo "<td align='center'><a href='bimbingan.php?id=".$row->id_pkt."' class='btn btn-info btn-s' role='button'>Tempatkan</a></td>";
										}else {
											echo "<td>".$row->nama_dosen."</td>";
										}
